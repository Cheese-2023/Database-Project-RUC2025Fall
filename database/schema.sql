...
CREATE TABLE employment_statistics (
    ...
    -- 计算字段（通过触发器或应用层计算）
    total_employment_万 DECIMAL(8,2) COMMENT '总就业人数(万人)',
...
CREATE TABLE economic_aggregate (
    ...
    industry_structure_1 DECIMAL(5,2) COMMENT '第一产业比重(%)',
    industry_structure_2 DECIMAL(5,2) COMMENT '第二产业比重(%)',
    industry_structure_3 DECIMAL(5,2) COMMENT '第三产业比重(%)',
    FOREIGN KEY (county_code) REFERENCES county_basic(county_code) ON DELETE CASCADE,
    UNIQUE KEY uk_county_year (county_code, year),
    INDEX idx_year (year),
...
CREATE TABLE fiscal_finance (
    ...
    -- 计算字段（通过触发器或应用层计算）
    fiscal_self_sufficiency DECIMAL(5,2) COMMENT '财政自给率(%)',
...
CREATE TABLE agriculture_production (
    ...
    -- 计算字段（通过触发器或应用层计算）
    mechanization_rate DECIMAL(5,2) COMMENT '农业机械化率(%)',
    grain_yield_per_hectare DECIMAL(8,2) COMMENT '粮食单产(吨/公顷)',
    FOREIGN KEY (county_code) REFERENCES county_basic(county_code) ON DELETE CASCADE,
    UNIQUE KEY uk_county_year (county_code, year),
    INDEX idx_year (year)
...
CREATE TABLE education_health (
    ...
    -- 计算字段（通过触发器或应用层计算）
    student_teacher_ratio_primary DECIMAL(5,2) COMMENT '小学生师比',
...
CREATE TABLE income_living_standard (
    ...
    -- 计算字段（通过触发器或应用层计算）
    income_gap_ratio DECIMAL(5,2) COMMENT '城乡收入差距比',
